...
//stop stalking my solution u mf!	
    int n,x1,y1,x2,y2;
    cin>>n>>x1>>y1>>x2>>y2;
    int firstclose=min(min(x1,y1),min(n+1-x1,n+1-y1));
    int secondclose=min(min(x2,y2),min(n+1-x2,n+1-y2));
    int legitpath=abs(x2-x1)+abs(y2-y1);
    int shortcut=firstclose+secondclose;
    if(shortcut<=legitpath){
        cout<<shortcut<<endl;
        return;
    }
    cout<<legitpath<<endl;
